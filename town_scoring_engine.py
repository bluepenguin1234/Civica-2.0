...
4 dimensions. Forked from scoring_engine.py: the county loaders are reused VERBATIM
...
POINTS = {
    ...
    'appr_quality':         (11.2, 'C'),
    'avg_wage':             (9.8,  'C'),
    'sector_quality':       (7.0,  'C'),
    'diversity':            (5.6,  'C'),
    'town_income_growth':   (5.6,  'T'),
    'violent':              (7.8,  'T'),
    'property':             (5.2,  'T'),
    'town_scale':           (5.2,  'T'),
    'amenity':              (3.9,  'C'),
    'physical_risk':        (3.9,  'C'),
    'town_growth_5yr':      (6.3,  'T'),
    'town_growth_relmom':   (4.5,  'T'),
    'net_migration':        (3.6,  'C'),
    'inmover':              (1.8,  'C'),
    'permits':              (1.8,  'C'),
}
T_POINTS = sum(p for p, layer in POINTS.values() if layer == 'T')   # 51.4
C_POINTS = sum(p for p, layer in POINTS.values() if layer == 'C')   # 48.6
...
def main():
    ...
    df['civica_score'] = df[['dim1', 'dim2', 'dim3', 'dim4']].sum(axis=1).clip(0, 100)
